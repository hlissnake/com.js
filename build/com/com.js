define("com/com", [ "./observer", "./class" ], function(require, exports, module) {
    // MC = window.MC || {};
    // MC.CObject = (function(MC, _){
    var Observer = require("./observer");
    var _COMEvents = ",touchend,touchmove,touchstart,click,mousemove,", _COMCustomEvents = ",tap,pinch,swipe,rotate,hold,";
    ...
    var COMObject = Observer.extend({
        ...
        _renderChildren: function(dt, context) {
            if (this.children) {
                ...
                var childCom, children = this.children, len = children.length;
                for (var i = 0; i < len; i++) {
                    childCom = children[i];
        ...
        _setGraphicStyle: function(context) {
            var translateX = this.x + this.width / 2, translateY = this.y + this.height / 2;
            if (this.transform) {
        ...
        on: function(ev, callback) {
            ...
            } else if (_COMCustomEvents.indexOf("," + ev + ",") > -1) {} else {
                me.supr(ev, callback);
            }
            return me;
        },
        ...
        _addEventListener: function(ev, trigger, callback, listenSelf) {
            var ev_map = this.ev_map, me = this;
            if (!ev_map[ev]) {
                ev_map[ev] = [];
        ...
        _eventTrigger: function(e) {
            var canvasOffset = this.canvas.getBoundingClientRect(), canvas = this.canvas, coordinate = {}, eventType = e.type, pageX, pageY;
            ...
            for (var i = 0; i < callbackList.length; i++) {
                var cb = callbackList[i], trigger = cb.t, target = trigger._findTarget(coordinate, cb.listenSelf);
                if (target) {
                    e.targetCom = target;
        ...
        _findTarget: function(coord, listenSelf) {
            var me = this, context = me.context, inPath, target;
    ...
    COMObject.Shape = {
        ...
        Circle: {
            draw: function(com, ctx) {
                var centerX = com.x + com.width / 2, centerY = com.y + com.height / 2, radius = com.width / 2;
        ...
        Bitmap: {
            draw: function(com, ctx, dt) {
                ...
                if (com.backgroundRepeatX) {
                    var w = com.backgroundImageWidth, h = com.backgroundImageHeight, cw = com.height / h * w, positionX = com.imagePositionX || 0, repeatNum = Math.ceil(com.width / cw) + 1;
