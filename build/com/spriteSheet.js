define("com/spriteSheet", [ "./observer", "./class" ], function(require, exports, module) {
    ...
    var SpriteSheet = Observer.extend({
        ...
        initialize: function(options) {
            ...
            if (this.frameData && !this.frames) {
                var frameData = this.frameData, fh = frameData.frameHeight, fw = frameData.frameWidth, ih = frameData.imgHeight, iw = frameData.imgWidth, num = frameData.frameNum;
                var frames = [], fCol = Math.floor(iw / fw), fRow = Math.floor(ih / fh), i, j = 0;
                for (;j < fRow; j++) {
                    for (i = 0; i < fCol; i++) {
                        var fd = [];
                        fd.push(i * fw);
        ...
        playAnim: function(name) {
            var me = this, animName = name ? name : this.firstAnim;
            if (this.animations) {
        ...
        draw: function(com, ctx, dt) {
            var frame = this.frames[this.frameIndex], sx = frame[0], sy = frame[1], swidth = frame[2], sheight = frame[3];
